#!/usr/bin/env bash
# Scenario 01: Platform Bootstrap — run.sh
# Orchestrates the full scenario: setup → assert → (teardown is handled by run-all.sh)
...
CLUSTER_NAME="${SCENARIO_CLUSTER:-chorister-scenario-01}"
SKIP_SETUP=0
SKIP_TEARDOWN=0
