#!/usr/bin/env bash
# Scenario 04: Network Isolation — run.sh
...
CLUSTER_NAME="${SCENARIO_CLUSTER:-chorister-scenario-04}"
SKIP_SETUP=0
SKIP_TEARDOWN=0
...
if [[ "$SKIP_SETUP" -eq 0 ]]; then
  # Scenario 04 requires Cilium — use setup-test-cluster.sh (not --plain-kind)
